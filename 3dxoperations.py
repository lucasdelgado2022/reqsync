...
def revise_serveral_reqs(ids:list[str]):
    global ENO_CSRF_TOKEN
    ...
    for index, parent in enumerate(parents):
        ...
        for child in childslists[index]:
            if count < 10:
                ...
                response = assign_child(session,ENO_CSRF_TOKEN,parent,childs)
                if not response.status_code == 200 and "ENO_CSRF_TOKEN" in response.json().get("message",""):
                    ENO_CSRF_TOKEN=getcsrf(session)
                    response = assign_child(session,ENO_CSRF_TOKEN,parent,childs)
                if not response.status_code == 200:
                    raise Exception("Error al asignar subrequerimientos",childs)
...
relate_several_elements(["padre1"],[["hijo1","hijo2"]])